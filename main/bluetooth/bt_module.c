...
BtDevice *btGetAvaibleDevices(int *deviceCount, connect_cb cb)
{
    ...
    vTaskDelay(BT_DISCOVERY_PERIOD_MS / portTICK_PERIOD_MS);

    printf("Stop discovery\n");
    esp_bt_gap_cancel_discovery();
    *deviceCount = avaibleDevicesCount;
...
static void bt_av_hdl_stack_evt(uint16_t event, void *p_param)
{
    ...
    case BT_APP_EVT_STACK_UP:
    {
        ...
        {
            int tmr_id = 0;
            s_tmr = xTimerCreate("connTmr", (BT_DISCOVERY_PERIOD_MS / portTICK_RATE_MS),
...
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
    ...
    size_t readSize;
    readSize = read(currentSongDescriptor, data, len); //fwave is a file
